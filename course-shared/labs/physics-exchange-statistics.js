(function (root, factory) {
  ...
  var STYLE_TEXT = [
    ".ex-lab{--ex-blue:#315f9d;--ex-gold:var(--cl-gold,#9b6a12);--ex-green:var(--cl-green,#39734d);--ex-red:var(--cl-red,#b64335);display:block;max-width:100%;min-width:0;color:var(--fg,#292722);line-height:1.55;overflow-wrap:anywhere}",
  ...
  function dashboardSvg(data, uid) {
    ...
    var baseline = 246, scale = 172;
    var parts = [
      '<svg viewBox="0 0 760 350" role="img" aria-labelledby="' + uid + '-title ' + uid + '-desc">',
      '<title id="' + uid + '-title">两个输入、两个输出与交换干涉概率</title>',
      '<desc id="' + uid + '-desc">左图蓝实线路径a到c和b到d，与红虚线路径a到d和b到c通向同一个符合事件。两组振幅相加，不能把线条当成独立概率。右图纵轴为0至1的概率。</desc>',
      text(190, 26, "符合事件的两组振幅", "ex-title", "middle"),
      '<rect class="ex-splitter" x="152" y="68" width="56" height="157" rx="5"></rect>',
      line(45, 96, 330, 96, "ex-input"), line(45, 198, 330, 198, "ex-input"),
      '<path class="ex-exchanged" d="M45 96 L135 96 L225 198 L330 198"></path>',
      '<path class="ex-exchanged" d="M45 198 L135 198 L225 96 L330 96"></path>',
      '<polygon class="ex-arrow" points="322,90 335,96 322,102"></polygon>',
      '<polygon class="ex-arrow" points="322,192 335,198 322,204"></polygon>',
      text(45, 80, "输入 a", "ex-label"), text(45, 220, "输入 b", "ex-label"),
      text(342, 100, "输出 c", "ex-label"), text(342, 202, "输出 d", "ex-label"),
      text(180, 61, "50/50", "ex-callout", "middle"),
      text(190, 252, semantics.distinguishable ? "可区分：两组路径概率各 1/4" : "直接项 −1/2；交换项 η/2", "ex-label", "middle"),
      text(190, 276, semantics.eigenvalueText, "ex-title", "middle"),
      text(190, 299, semantics.visibilityText, "ex-label", "middle"),
      text(190, 324, "红虚线：交换路由；线条不是粒子轨迹", "ex-label", "middle"),
      text(585, 26, "测量概率（固定 0–1 轴）", "ex-title", "middle")
    ];
    [0, 0.5, 1].forEach(function (p) {
      var y = baseline - scale * p;
      parts.push(line(458, y, 722, y, "ex-axis"), text(448, y + 4, String(p), "ex-label", "end"));
    });
    [[data.sameOutput, 514, "ex-same-bar", "同端 P_same"], [data.coincidence, 657, "ex-coin-bar", "符合 P_coin"]].forEach(function (bar) {
      var h = scale * bar[0];
      parts.push('<rect class="' + bar[2] + '" x="' + (bar[1] - 32) + '" y="' + (baseline - h) + '" width="64" height="' + h + '"></rect>',
        text(bar[1], baseline - h - 10, format(bar[0], 3), "ex-callout", "middle"), text(bar[1], 273, bar[3], "ex-label", "middle"));
    });
    parts.push(text(585, 309, "P_same+P_coin=1", "ex-label", "middle"), '</svg>');
    return parts.join("");
  }
  ...
  function mount(root, api) {
    ...
    root.innerHTML = [
      ...
      '</div>',
      '<p class="ex-note">窄屏可聚焦下图后左右滚动；概率柱共用固定刻度。左图仅显示符合通道的两组振幅。</p><div class="ex-stage" data-stage tabindex="0" role="region" aria-label="交换路由与概率图，可横向滚动"></div>',
      ...
      '<div class="ex-legend"><span><i class="ex-swatch"></i>同端输出</span><span><i class="ex-swatch ex-swatch-gold"></i>符合输出</span><span><i class="ex-swatch ex-swatch-red"></i>交换路由</span></div>',
      '</div></div>'
    ].join("");
    ...
    function render() {
      ...
      lab.querySelector("[data-status]").textContent = data.state.statistics === "distinguishable"
        ...
        : data.state.statistics === "boson" && data.state.overlap === 1
        ? "玻色子完全重合：交换路径与直接路径在符合通道相消，粒子成对出现在同一输出端。"
        : data.state.statistics === "fermion" && data.state.overlap === 1
    ...
    lab.addEventListener("click", function (event) {
      ...
      if (choice) {
        ...
        reveal.hidden = true;
        feedback.textContent = "预测已更改，请重新提交。";
