(function (root, factory) {
  ...
  function (host) {
    ...
    var STYLE_TEXT = [
      ...
      ".pse-lab .pse-choice-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}.pse-lab button{min-width:0;min-height:44px;padding:8px 11px;border:1px solid var(--border);border-radius:6px;background:var(--bg);color:var(--fg);font:inherit;line-height:1.35;cursor:pointer;overflow-wrap:anywhere}.pse-lab button:hover{border-color:var(--accent)}.pse-lab button[aria-pressed=true],.pse-lab button.pse-primary{border-color:var(--accent);background:var(--accent);color:var(--bg);font-weight:750}.pse-lab button:disabled{cursor:not-allowed;opacity:.55}.pse-lab [tabindex]:focus-visible,.pse-lab button:focus-visible,.pse-lab input:focus-visible{outline:3px solid var(--cl-focus,#1769aa);outline-offset:2px}",
      ...
      ".pse-lab .pse-revealed{margin-top:18px;padding-top:16px;border-top:1px solid var(--border)}.pse-lab .pse-layout{display:grid;grid-template-columns:minmax(0,1fr);gap:16px;align-items:start;min-width:0}.pse-lab .pse-controls,.pse-lab .pse-output{min-width:0}",
      ...
      ".pse-lab .pse-status,.pse-lab .pse-boundary{margin:10px 0;padding:10px 12px;border-left:3px solid var(--pse-blue);background:var(--bg);font-size:13px;line-height:1.7}.pse-lab .pse-boundary{border-left-color:var(--pse-gold)}.pse-lab .pse-chart-scroll,.pse-lab .pse-table-scroll{max-width:100%;overflow-x:auto;-webkit-overflow-scrolling:touch}.pse-lab .pse-chart-frame{min-width:0;padding:6px;border:1px solid var(--border);border-radius:6px;background:var(--bg)}.pse-lab svg{display:block;width:100%;height:auto;min-width:700px;max-width:none}.pse-lab svg text{fill:currentColor;font-family:inherit;letter-spacing:0}.pse-lab .pse-grid{stroke:var(--border);stroke-width:1;stroke-opacity:.75}.pse-lab .pse-axis{stroke:currentColor;stroke-width:1.1;stroke-opacity:.7}.pse-lab .pse-profile{fill:none;stroke:var(--pse-blue);stroke-width:3}.pse-lab .pse-reference{fill:none;stroke:var(--pse-gold);stroke-width:2;stroke-dasharray:6 4}.pse-lab .pse-bar{fill:var(--pse-green);opacity:.82}.pse-lab .pse-zero{stroke:var(--pse-red);stroke-width:1.4;stroke-dasharray:5 4}.pse-lab .pse-chart-label{font-size:11px}.pse-lab .pse-chart-title{font-size:13px;font-weight:750}",
    ...
    function presetById(id){var p=PRESETS.find(p=>p.id===id);if(!p)throw new Error("unknown PDE mode");return p;}
    function bounded(v,a,b,name){if(!finite(v)||v<a||v>b)throw new Error("invalid "+name);return v;}
    function modeIndex(n){bounded(n,1,4096,"mode index");if(!Number.isInteger(n))throw new Error("integer mode required");return n;}
    function normalizeParams(input){var q=input===undefined?{}:input;if(!q||typeof q!=="object"||Array.isArray(q))throw new Error("configuration object required");for(var k of Object.keys(q))if(!["mode","N","time","y"].includes(k))throw new Error("unknown parameter "+k);var p=Object.assign({},DEFAULTS,q);presetById(p.mode);bounded(p.N,1,MAX_N,"N");if(!Number.isInteger(p.N))throw new Error("integer N required");bounded(p.time,0,2,"time");bounded(p.y,0,PI,"height");return p;}
    function coefficient(n){modeIndex(n);return n%2===0?0:8/(PI*n*n*n);}
    function heatFactor(n,time){modeIndex(n);bounded(time,0,2,"time");return Math.exp(-n*n*time);}
    function waveFactor(n,time){modeIndex(n);bounded(time,0,2,"time");return Math.cos(n*time);}
    function laplaceFactor(n,y){modeIndex(n);bounded(y,0,PI,"height");if(y===0)return 0;if(y===PI)return 1;return Math.exp(-n*(PI-y))*(-Math.expm1(-2*n*y))/(-Math.expm1(-2*n*PI));}
    function modeFactor(mode,n,time,y){presetById(mode);return mode==="heat"?heatFactor(n,time):mode==="wave"?waveFactor(n,time):laplaceFactor(n,y);}
    ...
    function logAdd(a,b){if(a===-Infinity)return b;if(b===-Infinity)return a;var m=Math.max(a,b);return m+Math.log(Math.exp(a-m)+Math.exp(b-m));}
    function l2Tail(mode,N,time){
      if(mode==="laplace")return{lower:0,upper:0,remainderSquaredBound:0,logRemainderSquaredBound:-Infinity};
      var logSum=-Infinity;
      for(var n=N+1;n<=FULL_N;n++){if(n%2===0)continue;var logFactor=mode==="heat"?-2*n*n*time:2*Math.log(Math.abs(Math.cos(n*time)));logSum=logAdd(logSum,Math.log(32/PI)-6*Math.log(n)+logFactor);}
      // Integral test for all n>512 (including even n, hence conservative).
      var logBound=Math.log(32/(5*PI))-5*Math.log(FULL_N)-(mode==="heat"?2*(FULL_N+1)**2*time:0);
      return{lower:Math.exp(logSum/2),upper:Math.exp(logAdd(logSum,logBound)/2),remainderSquaredBound:Math.exp(logBound),logRemainderSquaredBound:logBound};
    }
    ...
    function evaluate(input) {
      ...
      for (var index = 0; index <= 320; index += 1) {
        ...
        points.push({
          ...
          reference: params.mode === "laplace" ? Math.sin(x) : x*(PI-x)
        });
      }
      var currentL2 = sumEnergy(params.mode, params.N, params.time, params.y, false);
      var tail=l2Tail(params.mode,params.N,params.time);
      var initialTruncated = sumEnergy(params.mode, params.N, 0, params.y, false);
      var initialFull = params.mode==="laplace"?initialTruncated:PI**5/30;
      ...
      return {
        ...
        points: points,
        currentL2: currentL2,
        currentL2TailLower:tail.lower,
        tailRemainderSquaredBound:tail.remainderSquaredBound,
        logTailRemainderSquaredBound:tail.logRemainderSquaredBound,
        initialL2Tail: Math.sqrt(Math.max(0, initialFull - initialTruncated)),
        currentL2Tail:tail.upper,
    ...
    function drawChart(doc, result) {
      ...
      var right = { x: 445, y: 32, width: 285, height: 246 };
      ...
      var extent=1.1*Math.max(1,...result.points.flatMap(p=>[Math.abs(p.value),Math.abs(p.reference)]));
      function py(value) { return left.y + left.height / 2 - value / extent * (left.height / 2 - 12); }
      [-1, 0, 1].forEach(function (fraction) {
        var tick=fraction*extent;var y = py(tick);
        svg.appendChild(svgNode(doc, "line", { x1: left.x, x2: left.x + left.width, y1: y, y2: y, className: "pse-grid" }));
        svg.appendChild(chartText(doc, left.x - 8, y + 4, format(tick,2), "pse-chart-label", { "text-anchor": "end" }));
      });
      ...
      svg.appendChild(chartText(doc, left.x + 10, left.y + left.height - 8, "蓝：当前有限解；金：精确初值/顶边", "pse-chart-label"));
      ...
      function by(value) { return right.y + right.height / 2 - value * (right.height / 2 - 22); }
      ...
      [-1,0,1].forEach(function(tick){svg.appendChild(chartText(doc,right.x-7,by(tick)+4,String(tick),"pse-chart-label",{"text-anchor":"end"}));svg.appendChild(svgNode(doc,"line",{x1:right.x,x2:right.x+right.width,y1:by(tick),y2:by(tick),className:"pse-grid"}));});
    ...
    function renderOutput(doc, output, result) {
      ...
      output.appendChild(element(doc, "div", "pse-metrics", [
        ...
        metric(doc, "初值/顶边 L² 尾", format(result.initialL2Tail, 5)),
        metric(doc, "当前 L² 尾区间", format(result.currentL2TailLower, 5)+" 至 "+format(result.currentL2Tail,5)),
      ...
      var status = result.params.mode === "heat"
        ? "热：尾项从 N+1 直接累加到512，再以积分判别法控制更高项；区间端点用浮点计算，避免两个大总量相减。"
        : result.params.mode === "wave"
          ? "波：L²尾区间用512项和剩余上界；能量尾另用解析总能量π³/6减去有限模态能量，包含位移梯度与速度。"
          : "Laplace：因子由 y/π 决定而非时间；本例顶边只有sin x，N≥1已完整，n>1的传递因子不代表实际激发。";
      ...
      setAttributes(chartScroll,{tabindex:0,role:"region","aria-label":"可横向滚动的模态图"});
      output.appendChild(chartScroll);
      output.appendChild(setAttributes(element(doc, "div", "pse-table-scroll", [makeTable(doc, result)]),{tabindex:0,role:"region","aria-label":"可横向滚动的模态账本"}));
    ...
    function mount(rootElement, api) {
      ...
      QUESTIONS.forEach(function (question) {
        ...
        question.options.forEach(function (option) {
          ...
          button.addEventListener("click", function () {
            ...
            state.revealed=false;revealedPanel.hidden=true;
            renderPrediction();
      ...
      function rangeControl(label, min, max, step, value, ariaLabel) {
        ...
        input.type = "range";
        input.id="pse-control-"+INSTANCE+"-"+ariaLabel.replace(/\s+/g,"-");labelNode.setAttribute("for",input.id);
      ...
      var yControl = rangeControl("内部高度 y/π",0,1,0.01,state.y/PI,"Laplace内部高度比");
      ...
      function renderPrediction() {
        QUESTIONS.forEach(function (question) {
          choiceButtons[question.id].forEach(function (choice) {
            ...
            choice.node.textContent=(state.revealed&&choice.id===question.answer?"✓ ":"")+question.options.find(o=>o.id===choice.id).label;
          });
        });
      }
      ...
      function render() {
        ...
        yControl.input.value = String(state.y/PI);
        timeControl.input.disabled=state.mode==="laplace";yControl.input.disabled=state.mode!=="laplace";
        ...
        renderOutput(doc, output, evaluate({mode:state.mode,N:state.N,time:state.time,y:state.y}));
      }
      ...
      reveal.addEventListener("click", function () {
        ...
        render();
        output.querySelector(".pse-chart-scroll").focus();
      ...
      yControl.input.addEventListener("input", function () {
        state.y = Number(yControl.input.value)*PI;
        if (state.revealed) render();
      });
      render();
    ...
    function selfTest() {
      ...
      check(result.points.length === 321, "profile sampling is deterministic");
    ...
    return {
      ...
      normalizeParams:normalizeParams,l2Tail:l2Tail,drawChart:drawChart,format:format,
