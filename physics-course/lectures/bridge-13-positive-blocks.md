...
取 h=1、z=1/2，只保留 n=0、1、2。三项分别是 $1/2,1/8,1/24$，于是

$$g_{1,2}(1/2)=\frac23,\qquad g'_{1,2}(1/2)=\frac74.$$

精确值却是 $\log2$ 与 2。正项级数意味着部分和从下方逼近，不能把“最后一项很小”直接叫作总误差。

本例首个遗漏项为 $t_3=1/64$，后续项比不超过 1/2，所以遗漏块值不超过 1/32。对导数，首个遗漏项为 1/8，相邻比不超过 5/8，遗漏导数不超过 1/3。因此得到可用分数核验的区间：

| 量 | 三项部分和 | 精确值所在区间 |
|---|---:|---|
| $g_1(1/2)$ | $2/3$ | $[2/3,67/96]$ |
| $g'_1(1/2)$ | $7/4$ | $[7/4,25/12]$ |

这只是手算练习，N=2 不在网页滑块范围内；它提前展示第6节余项界的逻辑。共形块的系数来自所有后裔，逐项截断的是计算，并未从理论中删除这些后裔。
...
一般背景见 [Simmons-Duffin，TASI讲义](https://arxiv.org/html/1602.07982v1)；OPE与共形块收敛、谱尾估计见 [Pappadopulo等，OPE Convergence](https://arxiv.org/html/1208.6449v3)。本页特定泛函和宽松h<4界在正文自行推导，不宣称是原文的最优结果。来源核查：2026-09-13。
