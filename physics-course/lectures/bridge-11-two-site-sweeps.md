...
取 $|\psi\rangle=|0\rangle(|0\rangle+i|1\rangle)/\sqrt2$，系数矩阵为

$$P=\frac1{\sqrt2}\begin{pmatrix}1&i\\0&0\end{pmatrix}.$$

$P^\dagger P$ 的非零本征向量可取 $v=(1,-i)^T/\sqrt2$，而右边的物理 ket 是 $\overline v=(1,i)^T/\sqrt2$。原因是 $P=U\Sigma V^\dagger$ 的展开系数包含 $V^\dagger_{a j}=\overline{V_{j a}}$。若误把 v 当作右 ket，本例中得到的右态与正确右态正交。实数实验看不出这个错误，所以推广复振幅时必须单独检查。

### 数值阈值会把精确等式变成近似等式

上面的 $WW^\dagger\psi_{\rm old}=\psi_{\rm old}$ 指保留全部非零支撑时的数学结论。实际删去小于阈值的权重后，应读取表中的投影误差，而不是假定它严格为零。记 $P_W=WW^\dagger$，在块基正交的前提下定义

$$\begin{gathered}\delta=\|(I-P_W)\psi_{\rm old}\|<1,\\
|\widehat\psi\rangle=\frac{P_W|\psi_{\rm old}\rangle}{\sqrt{1-\delta^2}}.\end{gathered}$$

$\widehat\psi$ 确实在搜索空间内，与旧态的保真度为 $1-\delta^2$。若完整 H 的谱位于 $[E_{\min},E_{\max}]$，上一讲的谱宽界给出

$$E_*\le E_{\widehat\psi}
\le E_{\rm old}+(E_{\max}-E_{\min})\delta.$$

因此投影误差非零时，“精确局部求解必不升能”要改成带误差的上界。δ 是范数误差，δ² 才是投影损失的权重；不要再多平方或少开一次方。本页各步直接核验 δ，实际轨迹中它接近舍入尺度。
...
假设旧能量 −4.50，未截断局部能量 −4.60，归一化截断候选 −4.48。局部求解改善了 0.10，但压缩损失了 0.12；候选比旧态高 0.02，开启规则时应拒绝。下一步继续使用旧态及 −4.50。不能把 −4.60 写进接受曲线，也不能用 −4.48 的候选去构造下一步环境。

| 这次尝试 | 未截断最优 | 截断候选 | 实际接受态 |
|---|---:|---:|---:|
| 拒绝规则开启 | −4.60 | −4.48 | −4.50（旧态） |
| 拒绝规则关闭 | −4.60 | −4.48 | −4.48（候选） |

这是接受规则的手算例；下节才是本页 Hamiltonian 的实际轨迹。先明确每条曲线记录哪个态，再读细小的数值变化。
...
<details markdown="1"><summary>一个零残差仍不是基态的最小反例</summary>

取 $H=\operatorname{diag}(0,1)$，态为 $(0,1)^T$。其能量 E=1，$(H-E)\psi=0$，但基态能量是 0。再取 $\psi_n=(-1)^n(1,0)^T$，能量始终为 0，向量却交替变号；物理纯态的密度矩阵完全相同。这说明讨论“态是否变化”还应去掉无物理意义的整体相位，例如比较密度矩阵、保真度或可观测量。对于存在简并的 H，相同能量的不同物理态也可以共存。
...
算法背景见 [Schollwöck 的 MPS 综述，第 6 节](https://arxiv.org/html/1008.3477v2)与 [两站点 DMRG 步骤](https://tensornetwork.org/mps/algorithms/dmrg/)。本页用独立 NumPy SVD 实现对照逐步轨迹，JavaScript 通过约化密度矩阵提取实块基；它们都使用同一明示的四站 Hamiltonian。下一步才是取消全态存储、实现缓存环境和迭代算符作用，以及更长链上的误差与初态敏感性训练。资料核查：2026-09-13。

现在可继续[真正存储 MPS：正交中心与环境缓存](bridge-12-mps-cache.html)，取消浏览器实验中的完整态存储，并比较逐步重建与缓存的计算量。
