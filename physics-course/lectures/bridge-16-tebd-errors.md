...
取两个归一化态 ψ、φ，已知 $\|\psi-\phi\|\le d$。对 Hermitian 观测 O，令其谱落在 $[o_{\min},o_{\max}]$，减去中点 $c=(o_{\min}+o_{\max})/2$ 不改变两态的期望值差。把差拆成两项，再用 Cauchy–Schwarz，得到
...
平均磁化 $\overline Z=L^{-1}\sum_jZ_j$ 的谱在 [−1,1]，所以差不超过 $\min(2,2d)$。如果经过有效的误差分析得到 d≤0.01，便能保证磁化误差不超过0.02；如果输入的是与小系统参照相比的实测距离，则得到的是该次比较的估计，不能自动升级成覆盖所有浮点误差的认证界。

这个界也可能很松。若 φ=−ψ，固定相位的态距离是2，但所有观测完全相同。采用适当的整体相位可以缩小距离；这就是网页同时报告不保真度的原因。另一方面，能量误差小只控制一个观测，无法反推出所有磁化与关联函数都准确。
...
MPS/TEBD实时演化与Strang分裂背景见 [Schollwöck的MPS综述第7节](https://arxiv.org/html/1008.3477v2)；更紧的对易子误差分析可读 [Childs等的Theory of Trotter Error with Commutator Scaling](https://arxiv.org/abs/1912.08854)。本页实际使用的保守常数由第5节独立推导，没有把更紧论文界未经实现地放进实验。资料核查：2026-09-13。
