...
定义奇偶算符 $P=\prod_j\sigma_j^z$。把全部自旋能级合并、**保留重数**并排序，完整隙是 $\Delta_{\rm spin}=E_1-E_0$；若基态简并，这个定义给零。只在 P=+1 子空间排序，得到偶扇区内部隙 $\Delta_+=E_{+,1}-E_{+,0}$。准粒子色散则给出第三个量

$$\epsilon_k=2J\sqrt{(g-1)^2+4g\sin^2(k/2)},\qquad k=\pi/L.$$
