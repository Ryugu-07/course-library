...
### 有限时间的曲线，为什么不能直接加权积到零频？

先固定有限的 $T$，将实际计算量记为 $\Gamma_T(\omega)=P^{(2)}(\omega,T)/T$。它一般不具有理想黄金律谱的低频空隙。以单个跃迁 $\hbar=\omega_{10}=1$ 为例，令 $\omega\to0$，余弦趋于 1，直接积分得

$$\Gamma_T(0)=\frac{4a^2|O_{10}|^2}{T}\sin^2(T/2).$$

例如 $T=\pi$ 时它严格为正。这是突然打开、持续有限时间的静态扰动对原本征态产生的跃迁估计，不需要假设系统真的有零能隙。连续性保证在足够小的正频率处它仍为正；所以此时

$$\int_0^b\frac{\Gamma_T(\omega)}{\omega^2}\,d\omega=\infty\qquad(b>0).$$

即使 $a$ 足够小、每个频率上的微扰概率都很小，这个带奇异权重的积分仍可发散。理想公式使用的是弱驱动窄峰极限得到的谱，不能交换成“任意有限脉冲曲线先积到零，再延长时间”。分布收敛也不自动允许用在零频奇异的 $1/\omega^2$ 作测试权重。

实际复算应登记正的频率下限、脉冲包络、背景处理与对时间窗的稳定性检查；一个偶然满足 $\sin(T/2)=0$ 的时长不能替所有协议消除问题。本页实验只画 $\omega\ge0.2$，表中的“理想加权谱面积”来自能级矩阵元公式，**没有把所画的有限时间曲线数值积分到零频**。上面的 $T=\pi$ 是区分极限顺序的手算输入，不在时长滑块的离散取值中。
