...
例如本实验在 $m=-1$ 使用中点网格近似曲率积分，得到 $C_-\approx+1$；应报告为 $+1$，同时保留“数值近似”的说明。$m=2.6$ 得到约 $5.7\times10^{-13}$，可报告为 $0$。在 $m=0$，高对称点检查给出 $\Delta=2.45\times10^{-16}$，物理上按零处理；此时不应把有限网格的 Chern 输出当作绝缘相标签。
