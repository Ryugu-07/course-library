...
请设计一个“由摆测 \(g\)”的记录页：列出 \(L,T\) 的原始重复读数、共同参考时钟对多次周期读数的相关项，以及有物理依据时长度与周期的交叉协方差、\(g\) 的传播矩阵，以及一个不依赖同一批数据的模型检查。然后把默认拟合数据改写成“有相关噪声”的情形：说明为什么应从逐点 \(1/\sigma_i^2\) 加权改为 \(\Sigma_y^{-1}\) 加权，并预测忽略正相关会怎样改变 \(\chi^2\) 与参数区间。最后给出一句可证伪结论，例如“在指定的独立高斯噪声和线性响应假设下，二次模型比线性模型更相容”，而不是“数据证明了二次规律”。
