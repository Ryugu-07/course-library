...
$$H=\begin{pmatrix}0&1\\1&2\end{pmatrix},\qquad q_1=\begin{pmatrix}1\\0\end{pmatrix}.$$

只取 K=1 时，Q 只有第一列，投影矩阵 B=(0)。它的最低本征对是 y=1、λ=0，投影残差当然为零；但恢复 v=q₁ 后，$Hv=(0,1)^T$，所以原问题残差是 1。

继续一次作用与正交化，得到 $q_2=(0,1)^T$。现在 Q 覆盖完整二维空间，最低 Ritz 值为 $1-\sqrt2$，恢复的向量才有零残差。

| 搜索空间 | 最低 Ritz 值 | 投影内部残差 | 原算符残差 |
|---|---:|---:|---:|
| 只有 q₁ | 0 | 0 | 1 |
| q₁、q₂ | $1-\sqrt2$ | 0 | 0 |

这一小例说明为什么要再次调用原来的 Hv 接口：投影内部的求解精度只回答小问题是否解好。它是独立手算例，网页扫描的最小步数仍为2。
...
MPO表达、作用与变分求解背景见 [Schollwöck 的MPS综述第5、6节](https://arxiv.org/html/1008.3477v2)。本页将算法限制在明示的实数/开放链模型，并以非Ising矩阵和独立NumPy扫描参照核验实现；这些有限验算不构成一般收敛定理。来源核查：2026-09-13。
