...
用本页参考单位固定 $s=1,g=1,t=-1/4,u=-3/4$。只为检验有理函数的留数账，作解析参数变化 $t(z)=-1/4+z,u(z)=-3/4-z$，于是始终有 $s+t+u=0$。这里的 $z$ 已用参考能量平方无量纲化；这份不变量计算不代替对外腿复动量的具体构造。得到

$$\begin{aligned}
\mathcal A(z)&=1+\lambda+\frac1{z-1/4}-\frac1{z+3/4},\\
\mathcal A(0)&=\lambda-\frac{13}{3}.
\end{aligned}$$

$\mathcal A(z)/z$ 在两个非零有限极点的留数分别是 $4$ 与 $4/3$，和为 $16/3$。另一方面，它在大 $z$ 的 $1/z$ 系数为 $1+\lambda$，所以无穷远留数为 $-(1+\lambda)$。包含原点在内的留数相加为零，给出

$$\mathcal A(0)=-\frac{16}{3}+(1+\lambda).$$

若删掉边界项，就把答案错误地写成 $-16/3$。值得自己检查的是：**即使取 $\lambda=0$，边界项仍有 1**，来自本次形变下不随 $z$ 变化的 $s$ 道交换项。因此不能只检查“有没有接触顶点”，还要检查哪些通道被选定形变移走、整个振幅在大 $z$ 怎样变化。这个静态算例没有加入角度滑块。
