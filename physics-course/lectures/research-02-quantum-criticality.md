...
在 $g=1$，令 $x=\pi/(2L)$，精确式给

$$\frac{L\delta_L}{2\pi J}=\frac{\sin x}{x}.$$

利用 $x-x^3/6\le\sin x\le x$，得到

$$\boxed{0\le1-\frac{L\delta_L}{2\pi J}
\le\frac{\pi^2}{24L^2}.}$$

例如L=8时，这个相对偏差至多约0.00643；L=32时至多约0.000402。这里的小偏差来自已知晶格色散修正，不需要通过调整临界指数来解释。

偏离临界后还可以精确比较两项：

$$\delta_L^2-\Delta_\infty^2
=16J^2g\sin^2\frac{\pi}{2L}.$$

对 $g\ne1$，只有当 $2\sqrt g\sin(\pi/(2L))\ll|g-1|$，最低动量尺度才接近热力学能隙；反向占优时，有限动量仍主导结果。长波下交叉大约发生在 $|g-1|L\sim\pi\sqrt g$。先识别自己处在哪个窗口，再判断一条下降曲线究竟在逼近有隙平台，还是临界的零隙极限。
