...
把另一个格点迹掉，在本地占据基 $|0\rangle,|\uparrow\rangle,|\downarrow\rangle,|\uparrow\downarrow\rangle$ 中，基态的约化密度矩阵是

$$\rho_1=\operatorname{diag}\left(
\frac D2,\frac{1-D}2,\frac{1-D}2,\frac D2\right).$$

原因可以直接从 $a|S\rangle+b|D_+\rangle$ 读出：$|b|^2=D$，两种双占据配置各占D/2；其余概率由两种分居自旋配置均分。每个本地配置都配着另一格不同的占据态，部分迹后交叉项消失。

用自然对数，单格与另一格之间的纠缠熵为

$$\boxed{\begin{aligned}
S(\rho_1)&=\log2-D\log D\\
&\quad-(1-D)\log(1-D).
\end{aligned}}$$

取 $0\log0=0$。$U=0$ 时 $D=1/2$，四个本地配置等概率，熵为 $\log4$；当 $U/t\to\infty$，熵趋于 $\log2$，留下自旋单重态的纠缠。因此，自旋反关联变强可以同时伴随这份**格点纠缠熵下降**：消失的是电荷涨落贡献，不能把所有关联指标都当成同一个量。

这一极限沿有限t的单重基态取到。若先严格设t=0，基态空间会简并，并不能自动选出同一个单重态。也不要把这里的完整格点约化熵替换成筛选双占据之后的条件熵；两者统计对象不同。
