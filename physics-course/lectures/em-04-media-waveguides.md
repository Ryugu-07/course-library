...
> 在自由空间里，平面波可以选择任意横向波矢；放进金属管后，横截面的边界条件会筛掉大多数场形状。结果不是“光变慢了”这么简单，而是一条精确的色散关系：低于截止频率，纵向波数变成虚数；高于截止频率，相速大于介质光速而群速小于它，两者的乘积由该理想模型的色散关系确定。

> 可直接复习：[Maxwell平面波约束](em-02-maxwell.html)、[能量流](em-03-energy-radiation.html)、[波包与色散](opt-01-waves.html)。
...
![默认TE10的正负磁场区与截止色散；颜色不表示连续场强。](assets/img/em-04-waveguide-default.svg)
<figcaption>默认TE10的正负磁场区与截止色散；颜色不表示连续场强。 图与下表采用相同默认参数。</figcaption>
</figure>
...
<details markdown="1"><summary>核对尺寸、介质和模式边界</summary>
...
实验颜色表示 $H_z$ 的正负区间，不是电场强度或连续幅值热图。TE$_{20}$的零点在$x/a=1/4,3/4$，三块宽度应为$1/4,1/2,1/4$，不能均分成三块。几何长宽比仅作示意；节点的归一化位置精确。可对照 [Fitzpatrick波导讲义](https://farside.ph.utexas.edu/teaching/em/lectures/node105.html)。
