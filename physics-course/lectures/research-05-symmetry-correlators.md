...
取默认 $m=1,a=1/2$，再加 $J_x=1,J_y=0$。此时均值 $\langle x\rangle=2/3$、$\langle y\rangle=0$，方差仍为 $C_x=2/3,C_y=2$；所以原始二阶矩是

$$\langle x^2\rangle=\frac{10}{9},\qquad
\langle y^2\rangle=2.$$

不能再把零源恒等式直接用于这些原始矩。变量代换应作用于有效指数 $S_a-J_xx$，其旋转变分多出 $J_xy$。对 $O=xy$，正确公式成为

$$\begin{aligned}
\langle x^2-y^2\rangle
&=-2am^2\langle x^2y^2\rangle\\
&\quad+J_x\langle xy^2\rangle.
\end{aligned}$$

两个方向仍高斯独立，故 $\langle x^2y^2\rangle=20/9$、$\langle xy^2\rangle=4/3$。右边为 $-20/9+4/3=-8/9$，恰与左边 $10/9-2$ 相同。若漏掉源项，就会得到一个假的非零残差。

本页实验只计算零源情形；这个额外例子用来训练检查条件：源打开时，不仅要区分原始矩与连通矩，还要让源本身参加变量代换。
