...
integer(c.n,1,512,"n");finite(c.threshold,0,512,"sum threshold");integer(c.events,1,10000,"events");finite(c.lambda,-20,20,"lambda");finite(c.singleThreshold,0,20,"single threshold");finite(c.p,0,1,"p");var canonical=Math.round(c.p*10000)/10000;if(Math.abs(c.p-canonical)>Number.EPSILON*Math.max(c.p,canonical))throw RangeError("p supports four decimal places");c.p=canonical;integer(c.count,0,512,"count");if(c.mode==="binomial"&&c.count>c.n)throw RangeError("count must not exceed n in binomial mode");integer(c.dimension,2,512,"dimension");if(c.dimension%2)throw RangeError("dimension must be even");finite(c.deviation,0,20,"deviation");return c;}
...
function read(){var o={},bad=null;lab.querySelectorAll("[data-key]").forEach(function(e){var k=e.getAttribute("data-key");if(e.tagName==="SELECT")o[k]=e.value;else{if(e.value.trim()===""||!e.validity.valid)bad=bad||e;o[k]=Number(e.value);}});if(bad){bad.closest("details").open=true;throw Error(bad.parentElement.firstChild.textContent+"：保留输入，请修正");}if(o.mode==="binomial"&&o.count>o.n){lab.querySelector('[data-controls="binomial"]').open=true;lab.querySelector('[data-controls="common"]').open=true;throw Error("计数阈值 k 不能超过试验数 n");}return config(o);}
