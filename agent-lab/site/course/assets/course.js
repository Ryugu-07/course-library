...
function buildEvalLab(root) {
  setMarkup(root, `
    <div class="lab-toolbar">
      <label>对照轴<select data-eval-axis><option value="model">model-only：固定 harness</option><option value="harness">harness-only：固定模型</option><option value="combined">combined：两者都换</option></select></label>
      ...
      <label>候选 harness 策略<select data-eval-strategy><option value="retry">重试 + 验证</option><option value="context">上下文 + 工具输出治理</option><option value="safety">沙箱 + 审批</option><option value="all">全部组合</option></select></label>
    </div>
    <div class="lab-output"><p class="lab-status" data-eval-scope></p><div class="eval-comparison"><div class="eval-version"><h3>基线 v0</h3><div data-eval-base></div></div><div class="eval-version candidate"><h3>候选 v1</h3><div data-eval-candidate></div></div></div><div class="release-decision" data-release-decision></div><div class="trace-box" data-eval-attribution></div><p class="lab-status">这里是确定性的教学模拟分数，不代表任何模型、harness 或产品基准。相同输入会重放同一账本。</p></div>
  `);
  ...
  function applyHarness(candidate,strategy,tasks){if(strategy==="retry"||strategy==="all"){candidate.success+=11;candidate.falseComplete-=9;candidate.cost+=.22;}if(strategy==="context"||strategy==="all"){candidate.success+=7;candidate.falseComplete-=4;candidate.cost-=.12;}if(strategy==="safety"||strategy==="all"){candidate.incidents=Math.max(0,candidate.incidents-Math.round(tasks*.035));candidate.cost+=.05;}}
  function render(){var axis=root.querySelector("[data-eval-axis]").value;var tasks=Number(root.querySelector("[data-eval-tasks]").value);var flaky=Number(root.querySelector("[data-eval-flaky]").value);var strategyControl=root.querySelector("[data-eval-strategy]");strategyControl.disabled=axis==="model";var strategy=strategyControl.value;var harnessChanged=axis==="harness"||axis==="combined";root.querySelector("[data-eval-task-value]").textContent=tasks;root.querySelector("[data-eval-flaky-value]").textContent=flaky+"%";var baseMetrics={success:Math.max(45,Math.round(72-flaky*.55)),falseComplete:18,incidents:Math.max(1,Math.round(tasks*.045)),cost:1};var candidate={success:baseMetrics.success,falseComplete:baseMetrics.falseComplete,incidents:baseMetrics.incidents,cost:1};var changes=[];if(axis==="model"||axis==="combined"){candidate.success+=8;candidate.falseComplete-=2;candidate.cost+=.18;changes.push("模型与推理配置");}if(harnessChanged){applyHarness(candidate,strategy,tasks);changes.push("agent harness 策略");}candidate.success=Math.min(96,candidate.success);candidate.falseComplete=Math.max(2,candidate.falseComplete);candidate.cost=Math.max(.7,Number(candidate.cost.toFixed(2)));renderRows(root.querySelector("[data-eval-base]"),baseMetrics);renderRows(root.querySelector("[data-eval-candidate]"),candidate);var scope=root.querySelector("[data-eval-scope]");if(axis==="model")scope.textContent="固定：提示、任务、agent harness、工具、权限、任务验收器、evaluation harness、环境镜像、预算与超时。改变：模型与推理配置。";else if(axis==="harness")scope.textContent="固定：模型、推理配置、提示、任务、任务验收器、evaluation harness、环境镜像、预算与超时。改变："+strategy+" agent harness 策略。";else scope.textContent="模型与 agent harness 同时改变：只能评价候选系统整体，不能从这组差值单独归因。";var failures={context:Math.max(0,Math.round(tasks*(0.08+(harnessChanged ? 0 : 0.02)))),tool:Math.max(0,Math.round(tasks*flaky/100*0.18)),runtime:Math.max(0,Math.round(tasks*(harnessChanged ? 0.025 : 0.06))),verification:Math.max(0,Math.round(tasks*candidate.falseComplete/100*0.35)),safety:Math.max(0,candidate.incidents)};root.querySelector("[data-eval-attribution]").textContent="失败归因（教学账本） · context "+failures.context+" · tool "+failures.tool+" · runtime "+failures.runtime+" · verification "+failures.verification+" · safety "+failures.safety+"。改变项："+changes.join(" + ")+"。";var pass=candidate.success>=baseMetrics.success+5&&candidate.falseComplete<=12&&candidate.incidents<=baseMetrics.incidents;var decision=root.querySelector("[data-release-decision]");decision.className="release-decision "+(pass?"pass":"hold");decision.textContent=pass?"建议进入小流量试运行：核心指标改善，风险没有恶化。":"暂缓发布：至少一个验收门槛没有通过。";}
