...
\le\gamma^{-2}\sum_k\frac{|f_k|^2}{\lambda_k}\lt \infty.
$$
...
<summary>展开完整答案：核方向是否进入测试空间</summary>
...
<details class="answer" markdown="1">
<summary>展开完整答案：从装配到整个区间积分</summary>
...
<details class="answer" markdown="1">
<summary>展开完整答案：每个系数先平方再求和</summary>

高频系数为 $u_k=\tau(-1)^{k+1}/(\pi^2k^4r_k)$，乘 $\lambda_k=\pi^2k^2$ 后，
$\lambda_k|u_k|^2=\tau^2/(\pi^2k^6r_k^2)$。在 $k>M$ 上，$0\lt r_-\le r_k\le r_+$，因此分别用 $r_+^2$、$r_-^2$ 控制分母，再对 $k^{-6}$ 作积分比较，即得两侧的五次幂界。
...
<details class="answer" markdown="1">
<summary>展开完整答案：角向边界、径向截断与导数积分</summary>
