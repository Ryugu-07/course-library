...
$$\begin{pmatrix}-g_{12}&1&0\\0&-g_{23}&1\\1&0&-g_{31}\end{pmatrix}
\begin{pmatrix}a_1\\a_2\\a_3\end{pmatrix}=0.$$

此矩阵行列式为 $1-h$。h=−1 时它在实数上可逆，所以只有零解；h=1 时秩为 2，核恰好一维。这把绕圈直觉与模、核、线性方程连接起来。若改用特征二的域，2 不再可逆，前一种推理也就不能照搬。

### 为什么负接缝的位置不重要？

在各弧上改用坐标 $a'_i=t_i a_i$，其中 $t_i\in\{\pm1\}$。代入 $a_j=g_{ij}a_i$ 得

$$g'_{ij}=t_jg_{ij}t_i^{-1}.$$

绕圈乘积中的中间因子两两抵消，留下 $h'=t_1ht_1^{-1}=h$。例如原符号 (−1,−1,+1)，取 $(t_1,t_2,t_3)=(1,-1,1)$ 就使三条新符号全为 +1。它是同一个局部系统的另一组坐标；若原 h=−1，就无法这样把三条全变正。这一结论针对本页秩一符号模型，更一般的局部系统需要处理矩阵共轭。
...
定义与函数例子参见 [Stacks Project：Sheaves](https://stacks.math.columbia.edu/tag/006S)。本讲的分式相容条件和三弧符号计算已逐步展开，不将它们当作一般几何定理的证明。资料核查：2026-09-13。
