...
**经济学读法（Kantorovich 的诺奖直觉）**：外包商对"从 $x$ 装货"收 $\varphi(x)$、"到 $y$ 卸货"收 $\psi(y)$，报价不超自运成本（约束）；对偶说**最优外包收入 = 自运最小成本**——影子价格的搬运版。**c-变换**：固定 $\varphi$ 后，可把 $\psi$ 提升到 $\varphi^c(y) = \inf_x[c(x,y) - \varphi(x)]$——这是随成本 $c$ 定制的广义共轭。它与 Legendre–Fenchel 共轭密切相关但不能无视符号。按 $g^*(z)=\sup_x\{\langle z,x\rangle-g(x)\}$ 的约定，当 $c(x,y)=\langle x,y\rangle$ 时

$$
\varphi^c(y)=\inf_x\{\langle x,y\rangle-\varphi(x)\}
=-(-\varphi)^*(-y),
$$

而不是 $-\varphi^*(-y)$；平方距离成本则在剥离二次项、重写势函数后化为通常的凸共轭（cvx-01）。
...
**定理** 在 $\mathbb{R}$ 上，若成本为 $c(x,y)=h(x-y)$ 且 $h$ 凸，则共同分位数耦合 $(F_\mu^{-1}(U),F_\nu^{-1}(U))$ 是一个最优方案。特别地，对 $p\ge1$ 的 $h(z)=|z|^p$，

$$
W_p^p(\mu,\nu) = \int_0^1\big|F_\mu^{-1}(t) - F_\nu^{-1}(t)\big|^p\,dt
$$

**【骨架】** 交换论证：若计划中存在“交叉”配对（$x_1<x_2$ 配 $y_1>y_2$），解开交叉不增成本（凸成本的四点不等式），反复交换即可构造共同分位数这一单调最优耦合。若 $h$ 严格凸，真正的交叉通常会使成本严格下降，因而可进一步排除最优计划中的交叉；若 $h$ 仅凸而有仿射区间，交换可能取等，其他非单调计划也可能同样最优。因此“存在单调最优解”和“所有最优解都单调”不能互换。上面的 $W_p$ 分位数公式专属于成本 $|x-y|^p$，不是任意凸 $h$ 的同名公式。$\blacksquare$
