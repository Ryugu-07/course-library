...
### 随机选一个 x，再选平方根，会均匀地选到曲线点吗？

取 $\mathbb F_5$ 上另一条光滑曲线 $y^2=x^3-x$。五列根数为 $1,1,2,2,1$，所以仿射点有 7 个，再加 $O$ 共 8 个。这个输入可在上方实验中设素数编号 0、$a=-1,b=0$ 检查。

若先以概率 $1/5$ 选一个 $x$，再在该列的根中均匀选 $y$，单根列里的每个点得到概率 $1/5$，双根列里的每个点却只有 $1/10$。这不是七个仿射点上的均匀分布，即使每一步看起来都在“均匀选择”。

要让仿射点均匀，可按该列根数除以 7 的概率选 $x$，再均匀选根；也可均匀选 $(x,y)\in\mathbb F_5^2$，只接受满足方程的候选。两者都让每个仿射点概率为 $1/7$。若目标是全部八点上的均匀分布，还须单独以 $1/8$ 选 $O$，以 $7/8$ 进入仿射采样。

逐列计数因此不只给出总数，还告诉我们不同采样方法实际给哪些点更大权重。本页仍是确定性计数实验，没有声称提供密码学随机点生成器。
