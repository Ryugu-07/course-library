...
**例 2（Chebyshev 界亲算）** $\kappa = 100$：收缩因子 $\frac{9}{11} \approx 0.818$。要让定理上界 $2(9/11)^k\le10^{-6}$，需

$$
k\ge \frac{\ln(2\cdot10^6)}{\ln(11/9)}\approx72.3,
$$

即至少 73 步。对照采用最优固定步长的梯度下降，同题的因子是 $99/101$，解 $(99/101)^k\le10^{-6}$ 得 $k\approx690.8$，即约 691 步。两者仍清楚展示 $\sqrt\kappa$ 对比 $\kappa$ 的量级差异；这些是最坏情形界，不是实际 CG 步数预言。
