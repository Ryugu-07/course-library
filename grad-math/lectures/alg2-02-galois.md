...
**与 at-02 的对表兑现**：中间域 ↔ 子群（反序）恰如覆盖空间 ↔ 子群；正规扩张 ↔ 正规子群恰如正规覆盖。这是强有力的**结构平行**，但不能仅凭一张逐行表就称为“范畴级同型”：覆盖分类还要固定合适的连通、局部路径连通且半局部单连通空间与基点，并明确覆盖映射、同构及共轭子群的口径；Galois 一侧则固定扩张并使用中间域嵌入。只有把对象、态射、基点与函子逐项指定后，才有资格讨论范畴等价。
...
**例 3（对表默写）** 不看书写出 Galois/覆盖空间对应的六行对照表（对象/态射/正规性/商/万有对象/“群”），再圈出哪些只是结构类比，哪些在补齐基点、态射和适当空间假设后能提升为严格分类定理。合格答案必须至少指出“子群本身”与“共轭类”的差别，不能只写“二者范畴同构”。$\blacksquare$
