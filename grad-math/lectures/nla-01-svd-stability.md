...
负特征值不会变成负奇异值。表内QR数值与下方QR静态图使用2026-09-11的Node 24.14.0 / macOS arm64运行快照；[下载原始矩阵、向量与逐点记录](assets/learning/projects/svd-stability/qr-snapshot.json)可独立核对。交互在当前浏览器实际计算，舍入误差的末位、接近机器精度的误差值可能不同。有理参考针对已经形成的浮点输入，理想数据的答案另列。后向误差实验直接指定候选解，不能据此断言某个求解器实际产生了它。
