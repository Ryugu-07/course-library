...
## 学习层：重尾、无均值与强依赖是三种不同的“不稳定”

<h3 id="lln-integrability-learning-title">1. 先预测：一般 i.i.d. SLLN 真正要哪一个矩？</h3>

实验并排回放四个模型：Bernoulli$(0.6)$、Pareto$(\alpha=1.5)$、标准 Cauchy，以及完全依赖序列 $X_i=Z\in\{-1,1\}$。先判断：

1. i.i.d. 强大数律是否必须要求有限方差？
2. Cauchy 对称于 $0$，样本均值是否应收敛到 $0$？
3. 每个 $X_i$ 都可积且同分布，若它们完全相同，平均是否仍自动收敛到 $EX_i$？

提交后，实验用固定 seed 展示运行均值并逐项点亮“同分布、独立、$L^1$、有限方差”四格证书。图只是条件后果的一条回放，不参与证明定理。

### 2. 静态后备：四个模型拆四个条件

<div class="learning-lab" data-learning-lab="lln-integrability" markdown="1">

| 模型 | $E\lvert X\rvert<\infty$ | 方差有限 | 独立 | i.i.d. $L^1$ SLLN |
|---|---|---|---|---|
| Bernoulli$(0.6)$ | 是 | 是 | 是 | 发证，均值趋 $0.6$ |
| Pareto$(\alpha=1.5)$ | 是，$EX=3$ | 否 | 是 | 仍发证，有限方差不是必要条件 |
| 标准 Cauchy | 否，均值未定义 | 否 | 是 | 不发证；$S_n/n$ 仍为 Cauchy |
| $X_i=Z$，$P(Z=\pm1)=1/2$ | 是 | 是 | 否 | 不发证；平均恒等于随机的 $Z$ |

Pareto 回放会偶尔被巨大观测拉动。它说明**收敛速度和有限样本稳定性**可能很差，却不否定 $E\lvert X\rvert<\infty$ 下的 a.s. 收敛。Cauchy 则连目标均值都没有；一条路径暂时靠近零不能创造期望。

完全依赖例子更直接：

$$
\frac1n\sum_{i=1}^nX_i=Z\quad\text{对每个 }n,
$$

所以它不会收敛到 $EZ=0$。可积性管单个变量的尾，独立性或其他遍历/弱依赖条件管平均能否洗掉共同随机性。

### 3. 0-1 律也不能越界

- Kolmogorov 0-1 律只说**独立序列的尾事件**概率属于 $\{0,1\}$，不告诉你该概率究竟是哪一个值，更不替代 SLLN 的矩条件证明。
- “收敛事件概率是 0 或 1”不等于“极限一定是常数”；还要结合尾可测性与适当的事件族或分布论证。
- 四阶矩证明是一个方便的充分条件证明，不应被误读为 SLLN 需要四阶矩。一般 i.i.d. 结论的精确门槛是 $L^1$。
- 固定 seed 使重尾反例可重放；它不会让一次有限模拟足以验证或推翻几乎处处命题。
