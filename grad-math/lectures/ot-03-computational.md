...
> OT 的计算革命：精确 LP 求解 $O(n^3\log n)$ 太贵，**熵正则化**把问题变成"矩阵缩放"——**Sinkhorn 算法**每步只做矩阵-向量乘，GPU 友好，把 OT 从理论请进了机器学习的日常。收官对账两大生成模型（WGAN、流匹配）。
...
第三重：$\varepsilon \to 0$ 收敛到精确 OT、$\varepsilon\to\infty$ 趋独立耦合——$\varepsilon$ 是"几何 vs 模糊"的旋钮。
...
——**交替满足两个边缘**（Gauss–Seidel 气质，cvx-03 的轮流坐庄第三次出现）。每步两次矩阵-向量乘：$O(n^2)$、可批量、GPU 天堂。

**定理（线性收敛）【骨架】** Sinkhorn 迭代在 **Hilbert 投影度量**（正锥上的射影距离 $d_H(x,y) = \ln\max_{i,j}\frac{x_iy_j}{x_jy_i}$）下是压缩映射：正矩阵 $K$ 的作用压缩系数 $\lambda(K) = \frac{\sqrt{\eta}-1}{\sqrt\eta+1} < 1$（$\eta$ 与 $K$ 的元素比有关）⇒ 几何收敛。$\blacksquare$
...
**统计红利【引用】**：熵正则化的样本复杂度从 $n^{-1/d}$（ot-02 的维数灾难）改善到 $n^{-1/2}$ 级（依赖 $\varepsilon$）——**正则化同时买计算与统计**（这个双赢在 ML 里似曾相识：岭回归/早停——正则化哲学的又一票）。Sinkhorn 散度（去偏版）修正熵正则的偏移【引用】。
