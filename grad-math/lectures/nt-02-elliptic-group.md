...
在 $\mathbb F_5$ 上比较两条光滑曲线 $E_0:y^2=x^3-x$ 与 $E_1:y^2=x^3-x+1$。逐列计数均得 8 个点，含 $O$。但 $E_0$ 有三个非零 2 扭点 $(0,0),(1,0),(4,0)$；$E_1$ 只有 $(3,0)$ 一个。

用本讲加法公式可算出：在 $E_0$ 中，$P=(2,1)$ 满足 $2P=(0,0),4P=O$；取 $Q=(1,0)$，八个 $jP+kQ$（$0\le j<4,0\le k<2$）互不相同，故

$$E_0(\mathbb F_5)\cong\mathbb Z/4\mathbb Z\times\mathbb Z/2\mathbb Z.$$

在 $E_1$ 中，$R=(0,1)$ 满足 $2R=(4,1),4R=(3,0),8R=O$。由于 $4R\ne O$，其阶为 8，所以 $E_1(\mathbb F_5)$ 是循环群。

两者有相同点数、相同迹 $a_5=-2$，却没有相同群结构。计数本身不能判定是否循环，也不能恢复全部有理扭点。下方实验固定的是原来模 17 的曲线；本段的两条模 5 曲线需手算，或使用第一讲计数器并另行实现加法，不能只改变倍点整数来切换曲线。
