...
### 三次项均值本来就是零，为什么仍要减去 3CX？

若只要求中心化，$X^3$ 已经满足 $\mathbb E X^3=0$。但它仍混有与线性变量 $X$ 同方向的部分。把随机变量看成带内积 $\langle U,V\rangle=\mathbb E[UV]$ 的向量，寻找最接近 $X^3$ 的线性项 $\alpha X$：

$$\begin{aligned}
\mathbb E[(X^3-\alpha X)^2]
&=15C^3-6\alpha C^2+\alpha^2C\\
&=6C^3+C(\alpha-3C)^2.
\end{aligned}$$

$C>0$ 时唯一最优系数为 $\alpha=3C$，余项 $:X^3:$ 与 $X$ 正交，因为 $\mathbb E[X(X^3-3CX)]=3C^2-3C^2=0$。这给出了低阶反项的一个可检查理由：它去除指定 Gaussian 内积中的低阶成分，而不仅是把均值平移到零。

$C=2$ 时，原 $X^3$ 的方差为 120，投影项 $6X$ 的方差为 72，余项方差为 48；正交性使 $120=72+48$。投影减少这个单变量的均方大小，仍不保证随截断增长的场会收敛。

<details markdown="1">
<summary>迁移核对：仍用方差 C 的变量，却错用反项 3(C+c)X 呢？</summary>

余项变成 $:X^3:-3cX$，与 $X$ 的内积为 $-3cC$，不再正交；其方差为 $6C^3+9c^2C$。有限反项的选择改变了低阶成分。这里的投影解释针对单个 Gaussian 三次多项式，不能把完整奇异 SPDE 的所有重整化简化为一次回归。
