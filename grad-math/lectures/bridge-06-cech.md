...
对上面的 k=−3 例子，选 $f_0(z)=-7-z^2$、$f_1(w)=2w$。按本讲符号约定，

$$\delta(f_0,f_1)=2z^{-4}+7+z^2.$$

所以 $h-\delta(f_0,f_1)=3z^{-2}+5z^{-1}$。这对局部多项式就是“两端可消去”的具体见证；剩下两项不能由任一侧允许的指数产生。注意 f₀ 前面的负号：微分定义为第二份数据减第一份数据，不能只凭图形记一个模糊的“做差”。

全局截面的两份系数也能逐项对应。k≥0 时，$f_0(z)=z^j$ 对应 $f_1(w)=w^{k-j}$，其中 $0\le j\le k$。例如 k=2 的三组局部系数是 (1,w²)、(z,w)、(z²,1)。这些是 $\mathcal O(2)$ 截面的局部系数，不是射影直线上的三个全局正则函数；$\mathcal O$ 自身的全局正则函数在本例中只有常数。

| 线丛次数 k | $h^0$ | $h^1$ | 直接检验的现象 |
|---|---:|---:|---|
| −3 | 0 | 2 | 无非零全局截面，但有两维一次类 |
| −1 | 0 | 0 | 两种空间都为零 |
| 0 | 1 | 0 | 常数全局截面 |
| 2 | 3 | 0 | 三组可粘合的局部多项式 |

这四行能检查自己有没有把“没有全局截面”误说成“一定有粘合障碍”。
...
$\delta=z^kf_1-f_0$；$H^0$ 数两个指数区域的交集，$H^1$ 数未被并集覆盖的缺口。一般定理与射影空间计算参见 [Stacks：Cohomology of projective space](https://stacks.math.columbia.edu/tag/01XS)。本讲仅在 $\mathbb P^1_{\mathbb C}$ 上展开计算，资料核查：2026-09-13。
