...
做一个**假设的校准扰动实验**，不改 NOAA 原始资料，也不声称它存在下述漂移。设设计矩阵 $X$ 满列秩，把每个观测加上落在模型空间内的偏差 $X\delta$：$y'=y+X\delta$。最小二乘的线性关系给

$$\widehat\beta'=\widehat\beta+\delta,\qquad y'-X\widehat\beta'=y-X\widehat\beta.$$

残差完全相同；设计矩阵也没变，所以本课由残差计算的 iid 与 HAC 协方差都相同。以默认中心年 2015 为例，给训练和留出每个月都加上 $0.1(t-2015)$ ppm，斜率会从约 $2.428153$ 变成 $2.528153$ ppm/年，HAC 标准误仍约 $0.036299$ ppm/年。这个 $0.1$ 是人为设定的斜率偏差，不是从数据估计的发现。

为何留出检验也可能看不见？模型向 2020—2021 年延伸时，同样多预测了 $0.1(t-2015)$；若留出读数共享这项假设偏差，观测减预测仍不变。因此留出检验能检查相对于这条测量链的预测能力，却不能单独校准测量链本身。把训练期资料锁定和用独立标准检查仪器，是不同的证据。

<details markdown="1">
<summary>迁移核对：只给所有月份加同一个 1 ppm，会改变什么？</summary>

本课所有模型都含截距，因此仅截距加 1 ppm，斜率、残差和这些残差型标准误不变。若偏差只在留出期突然出现，则不再是训练与留出共享同一 $X\delta$ 的情形，留出误差通常会改变。上述不变性有明确条件，不能泛化为“系统误差永远无法发现”。
