(function (root, factory) {
  ...
  function bernoulliLedger(nValue, pValue) {
    ...
    return { n: n, p: p, distribution: distribution, mean: mean, variance: distribution.reduce(function (sum, row) { return sum + row.probability * (row.estimate - mean) ** 2; }, 0), asymptoticVariance: p * (1 - p) / n, regular: p > 0 && p < 1, fisherPerObservation: p > 0 && p < 1 ? 1 / (p * (1 - p)) : Infinity };
  }
  function waldInterval(k, n, z) { var phat = k / n; var se = Math.sqrt(phat * (1 - phat) / n); return [Math.max(0, phat - z * se), Math.min(1, phat + z * se)]; }
  function wilsonInterval(k, n, z) { var phat = k / n; var z2 = z * z; var denominator = 1 + z2 / n; var center = (phat + z2 / (2 * n)) / denominator; var half = z * Math.sqrt(phat * (1 - phat) / n + z2 / (4 * n * n)) / denominator; return [k === 0 ? 0 : Math.max(0, center - half), k === n ? 1 : Math.min(1, center + half)]; }
  function exactCoverage(nValue, pValue, method) {
    ...
    for (var k = 0; k <= n; k += 1) { var interval = method === "wilson" ? wilsonInterval(k, n, z) : waldInterval(k, n, z); if (p >= interval[0] && p <= interval[1]) coverage += binomialPmf(k, n, p); }
    return coverage;
  }
  function selfTest() {
  ...
  var STYLE_TEXT = [
    ...
    ".ma-lab .ma-layout{display:grid;grid-template-columns:minmax(0,1fr);gap:16px;align-items:start;min-width:0}.ma-lab .ma-controls{display:grid;gap:12px;padding:12px;border:1px solid var(--border);border-radius:7px;background:var(--bg)}.ma-lab .ma-control{display:grid;gap:4px}.ma-lab label{color:var(--fg-soft);font-size:13px;font-weight:700}.ma-lab output{color:var(--accent);font-variant-numeric:tabular-nums}.ma-lab input[type=range]{width:100%;min-height:44px;margin:0;accent-color:var(--accent)}",
    ".ma-lab .ma-stage{min-width:0;padding:9px;border:1px solid var(--border);border-radius:7px;background:var(--bg);overflow:hidden}.ma-lab .ma-scroll{overflow-x:auto}.ma-lab svg{display:block;width:100%;height:auto;min-width:640px;color:var(--fg)}.ma-lab svg text{fill:currentColor;font-family:inherit;letter-spacing:0}.ma-lab .ma-axis{stroke:currentColor;stroke-width:1.2;opacity:.7}.ma-lab .ma-bar{fill:var(--ma-blue)}.ma-lab .ma-normal{fill:none;stroke:var(--ma-gold);stroke-width:3}.ma-lab .ma-true{stroke:var(--ma-red);stroke-width:2;stroke-dasharray:5 4}",
  ...
  function mount(container) {
    ...
    container.innerHTML = [
      ...
      '<div class="ma-question" data-question="2"><strong>3. 95% Wald 区间是否在所有 n,p 上精确覆盖 95%？</strong><div class="ma-choices"><button type="button" data-choice="0">是</button><button type="button" data-choice="1">只在 p=1/2</button><button type="button" data-choice="2">否，覆盖率有跳跃且边界可失真</button></div></div>',
      ...
      '<div class="ma-stage"><div class="ma-scroll" role="region" tabindex="0" aria-label="MLE分布图；窄屏可横向滚动"><svg viewBox="0 0 640 310" role="img" aria-labelledby="' + prefix + '-title ' + prefix + '-desc"><title id="' + prefix + '-title">Bernoulli MLE 精确分布与正态近似</title><desc id="' + prefix + '-desc">蓝色柱是二项精确质量，金线是正态密度乘网格间距1/n，仅作局部质量近似，可能大于1。</desc><g data-svg></g></svg></div>',
    ...
    function render() {
      ...
      var maxMass = Math.max.apply(null, ledger.distribution.map(function (row) { return row.probability; }));
      var peak = ledger.regular ? normalDensity(p, p, sd) / n : 0;
      var yMax = 1.12 * Math.max(maxMass, peak), left = 60, right = 604, bottom = 258, top = 58;
      function xx(x) { return left + (right - left) * x; }
      function yy(y) { return bottom - (bottom - top) * y / yMax; }
      var elements = ['<line class="ma-axis" x1="60" y1="258" x2="604" y2="258"></line><line class="ma-axis" x1="60" y1="58" x2="60" y2="258"></line>'];
      for (var tick = 0; tick <= 4; tick++) { var level = yMax * tick / 4; elements.push('<text x="52" y="' + (yy(level)+4) + '" text-anchor="end" font-size="12">'+format(level,3)+'</text>'); }
      [0,.25,.5,.75,1].forEach(function(v){elements.push('<text x="'+xx(v)+'" y="280" text-anchor="middle" font-size="12">'+v+'</text>');});
      ledger.distribution.forEach(function (row) { var width = Math.min(14, 500 / (n + 1)); var y = yy(row.probability); elements.push('<rect class="ma-bar" x="' + (xx(row.estimate) - width / 2).toFixed(2) + '" y="' + y.toFixed(2) + '" width="' + width.toFixed(2) + '" height="' + (bottom-y).toFixed(2) + '"></rect>'); });
      if (ledger.regular) { var curve = [], samples=[]; var count=Math.max(300,Math.ceil(24/sd));for(var i=0;i<=count;i++)samples.push(i/count);samples.push(p);samples.sort(function(a,b){return a-b});samples.forEach(function(estimate,index){var y=yy(normalDensity(estimate,p,sd)/n);curve.push((index ? "L" : "M")+xx(estimate).toFixed(2)+" "+y.toFixed(2));});elements.push('<path class="ma-normal" d="'+curve.join(" ")+'"></path>'); }
      var trueX=xx(p);elements.push('<line class="ma-true" x1="'+trueX+'" y1="58" x2="'+trueX+'" y2="258"></line><text x="60" y="22" font-size="12">柱：精确概率质量；金线：正态密度 / n；红线：真值</text><text x="60" y="42" font-size="12">纵轴随参数缩放，窄屏可横向滚动</text><text x="604" y="304" text-anchor="end" font-size="12">估计值 p̂</text>');lab.querySelector('[data-svg]').innerHTML=elements.join('');
      ...
      lab.querySelector("[data-note]").textContent = ledger.regular ? "当前真值是内点。金线是密度乘格距1/n，仅在局部近似平坦时近似一个格子的质量；小n、近边界时它可能超过1，不能当作概率。有限n仍离散，区间覆盖不逐点等于0.95。" : "当前真值位于参数空间边界，样本退化且正则渐近正态性停止发证；增加 n 不能把边界变成内点。";
    }
