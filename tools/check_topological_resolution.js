...
for(let im=0;im<=64;im++){const m=-3.2+im*.1,gap=a.bulkGap(m);for(let ix=0;ix<=32;ix++)for(let iy=0;iy<=32;iy++){let x=-Math.PI+ix*Math.PI/16,y=-Math.PI+iy*Math.PI/16;let n=2*Math.hypot(Math.sin(x),Math.sin(y),m+Math.cos(x)+Math.cos(y));ok(n+1e-12>=gap,'global analytic minimum')}}
