...
某团队测试 40 个指标，只报告最小的 $p=0.018$；另一个团队只测试一个预先注册指标，得到同样的 p 值。两项数据的单项 z 分数绝对值相同，为什么证据账本不同？若第二个团队还规定“提升至少 0.2 个标准差才值得上线”，你还需要哪些区间信息？

<details markdown="1">
<summary>迁移题核对：把筛选过程写进错误率</summary>

取家族水平 $0.05$，40 项 Bonferroni 阈值为 $0.05/40=0.00125$，$0.018$ 未达标；对应调整 p 值为 $\min(1,40\cdot0.018)=0.72$。一个预注册单项检验则在水平 $0.05$ 下拒绝。相同的单项尾概率没有包含相同的选择过程。

若关心正向提升至少 $0.2$，需要匹配模型的区间下端超过 $0.2$，以及预先规定的区间水平；只知双侧 p 值，连效应正负都不知道，也不能知道效应量、标准误和样本量各是多少。区间完全落在 $[-0.2,0.2]$ 内与“未拒绝零效应”也不同；若要正式做等效检验，还需对应双单侧规则。

先修：[抽样分布](stat-01-sampling.html)、[置信区间](stat-03-interval.html)、[Bayes 条件方向](prob-01-space-bayes.html)；后续：[回归诊断](stat-05-regression.html)。
...
做 $m$ 个水平至多 $\alpha$ 的空检验时，$E[V]\le m\alpha$；若每项实际错误率都等于 $\alpha$ 才取等号，再加独立性才有 $P(V\ge1)=1-(1-\alpha)^m$。Bonferroni 用联合界控制家族错误率，不需要独立性。

Benjamini–Hochberg（BH）控制另一目标 $\mathrm{FDR}=E[V/\max(R,1)]$，其中 $V$ 为错拒数、$R$ 为总拒绝数。排序 $p_{(1)}\le\cdots\le p_{(m)}$，找最大 $k$ 使 $p_{(k)}\le kq/m$，拒绝前 $k$ 项；没有这样的 $k$ 就不拒绝。有效 p 值加独立性，或真零假设子集上的 PRDS 正依赖条件，给出 FDR 不超过 $q$ 的保证；任意相关性不能直接沿用。任意依赖下可用更保守的 BY 阈值 $kq/(m\sum_{j=1}^m1/j)$。条件与结论见 [Benjamini–Yekutieli 原文](https://doi.org/10.1214/aos/1013699998)。选择方法须先明确要控制“至少错一次”还是“发现中错的比例”。
