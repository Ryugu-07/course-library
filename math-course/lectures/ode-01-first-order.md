...
1. 给 $y'=y^2,\ y(0)=y_0$ 写出解及最大区间；$y_0=0$ 是否会被分离变量漏掉？
2. 对 $y'=\sqrt{|y|}$，证明等待解在 $t=a$ 也是 $C^1$ 解。Euler 从精确零出发为什么不能发现所有分支？
3. $y'+y/x=\sin x/x$ 的显式解如果在 $x=0$ 有有限极限，是否就成为跨过零点的原方程解？

<details markdown="1"><summary>展开核对：初值、拼接和最大区间</summary>

1. $y_0=0$ 时 $y\equiv0$；否则 $y=y_0/(1-y_0t)$。当 $y_0>0$，最大区间 $(-\infty,1/y_0)$；当 $y_0<0$，是 $(1/y_0,\infty)$。每次除以 $y^2$ 后必须补回零解。
2. $t<a$ 时导数与右端均为零；$t>a$ 时均为 $(t-a)/2$；拼接点两侧导数都趋零。Euler 更新 $y_{n+1}=y_n+h\sqrt{|y_n|}$ 从零始终为零，所以只能选择一个合法分支，不能作为唯一性证据。
3. 不能。即使选 $C=1$ 使 $(1-\cos x)/x\to0$，原方程的系数仍在零点无定义。若要延拓，须另行定义新方程，并重新核对其解。

</details>

先修：[微积分基本定理](analysis-03-integral.html)、[完备性与压缩映射](func-01-banach.html)；后续：[线性系统稳定性](ode-03-systems-stability.html)。一般存在和延拓条件见 [Teschl 第 2 章](https://www.mat.univie.ac.at/~gerald/ftp/book-ode/ode.pdf)。
