...
对 heavy-ball 更新 $x_{k+1}=x_k-\alpha Hx_k+\beta(x_k-x_{k-1})$，沿 $H$ 的特征向量、特征值 $\lambda$ 投影后有 $z_{k+1}=(1+\beta-\alpha\lambda)z_k-\beta z_{k-1}$，故特征多项式是 $r^2-(1+\beta-\alpha\lambda)r+\beta=0$。稳定要对谱中的每个 $\lambda$ 检查两个根都严格落在单位圆内；只检查一个平均曲率不够。若使用近似 Hessian 或近似线性求解，还要把谱误差和求解残差纳入收缩界。
