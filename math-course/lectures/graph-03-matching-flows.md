...
对实验里的最终最小割任选一条割边，把容量增加 1，再预测最大流是否一定增加 1。随后把一条**不在任何当前最小割上**的边增加 1，比较结果。答案是：第一种未必增加，因为另一张同值最小割可能完全没有变化；第二种一定不会仅靠这次扩容提高最大流，因为至少一张原最小割仍以原容量给出上界，而原最大流仍然可行。由此说明：瓶颈边给出局部的容量影子价格线索，但最小割可能不唯一，改动后瓶颈也会迁移；“某条边在一张最小割上”不能单独保证任意幅度扩容都等量提升系统吞吐。
...
（"任何 $k$ 个姑娘合起来认识至少 $k$ 个小伙"——没有任何一群人被挤在过窄的选择里。）必要性显然；充分性的经典证法是**增广路**：未匹配点出发交替走"非匹配边/匹配边"，找到增广路则翻转它使匹配 +1。若还要求完美匹配，必须让匹配也覆盖 $Y$；有限二部图中通常要先有 $|X|=|Y|$，此时覆盖 $X$ 就自动覆盖两侧。这个算法思想同时是匈牙利算法与下节最大流的引擎。
