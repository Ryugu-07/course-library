...
🔗 **AI 衔接（收网）**：comfy 课 03 讲说"采样器 = 概率流 ODE 的数值积分器"——现在你有了全部零件：`euler` 对应一阶显式推进、`heun` 对应二阶预测—校正，`dpm++` 则利用扩散方程结构设计更新。实践中“20–30 步看起来够用”不能仅凭少量试跑反推出高阶收敛：还要固定模型、噪声日程与误差指标，做步长细化对照，并区分离散误差、模型误差和感知指标。Karras 日程是非均匀预设网格，与依据在线局部误差估计调整步长的经典自适应算法相关，但不完全等同。**Neural ODE** 则反过来把 ODE 求解器当网络层反传。
