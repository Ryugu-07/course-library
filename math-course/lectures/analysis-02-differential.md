...
<section class="learning-layer" markdown="1" aria-labelledby="analysis-02-learning-title">
...
$$
|f(a+h)-L_a(a+h)|\leq \frac{M}{2}h^2,\qquad
\left|\frac{f(a+h)-f(a)}h-f'(a)\right|\leq \frac{M}{2}|h|.
$$

前一条是**函数值的线性化误差**，后一条是**割线斜率对导数的误差**；不要把“差商趋于导数”误读成任意有限 $h$ 已经等于导数。

### 2. 三项预测：先写下阶数与边界

1. 当 $f''$ 在 $a$ 附近有界时，割线斜率与 $f'(a)$ 的误差是 $O(|h|)$ 还是 $O(h^2)$？
2. 同一条件下，$f(a+h)$ 与切线 $L_a(a+h)$ 的误差是 $O(|h|)$ 还是 $O(h^2)$？
3. 对 $|x|$ 在 $0$ 与 $x^2$ 在 $0$，能否不加条件地使用“导数”和“反函数导数为 $1/f'$”的口诀？
...
<div class="learning-lab" data-learning-lab="derivative-local-linearity" markdown="1">
...
1. 对 $f(x)=\sin x$ 在 $a=0$ 的切线，函数值误差和割线误差各是什么阶？是否与上面的二阶上界矛盾？
2. 为什么 $f(x)=x$ 在 $[0,1]$ 的最小值点 $0$ 不满足 $f'(0)=0$？
3. 设 $f'(x)/g'(x)$ 没有极限，应该立即放弃求 $f/g$ 的极限吗？给出本页之外的例子。

<details markdown="1"><summary>展开核对：上界不要求每个模型都恰好达到</summary>

1. $\sin h-h=-h^3/6+O(h^5)$，故函数值误差为 $O(|h|^3)$，割线误差为 $O(h^2)$。这比一般 $C^2$ 模型的上界更小；因 $f''(0)=0$，二阶项消失。
2. $0$ 是端点；只有一侧可行扰动，Fermat 的两侧差商夹逼不适用。
3. 取 $f(x)=x+\tfrac12\sin(2x)$、$g(x)=x$，当 $x\to+\infty$，原比趋于 $1$，导数比 $1+\cos(2x)$ 不收敛。法则的充分条件失效，不等于原极限不存在。

</details>

先修回链：[极限与连续](analysis-01-real-limits.html)；继续到[多元线性化](analysis-05-multivar-diff.html)和[求根停止条件](num-03-roots-interp.html)。
