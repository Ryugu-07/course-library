...
**定理** 在单物品、私人价值、准线性效用、无预算/外部性约束且支付规则确实为第二价格的标准模型中，**如实出价是（弱）优势策略**。这个结论不需要竞标者估值相互独立；独立同分布等假设常用于求最优保留价或期望收入，不是这条优势策略证明的前提。
*证明（两行）*：你的出价只决定"是否赢"，不决定"付多少"（付的是别人的价）——报高只会在"估值 < 第二价"时多赢下亏本局，报低只会错失有利可图的局；如实出价恰好"该赢则赢"。$\blacksquare$

**读法**：好机制能让说真话成为最优，但名称相近不代表激励性质相同。**VCG** 是把外部性支付推广到多物品/公共决策的一族机制；在标准准线性私人价值环境下，它与单物品 Vickrey 拍卖一样具有优势策略真实性。**GSP**（广义第二价格）按下一名出价给多个广告位定价，通常并不让如实出价成为优势策略，只能在附加假设下讨论其均衡。把 GSP 与 VCG 并写成“二价机制变体”不能继承上面的两行证明。
