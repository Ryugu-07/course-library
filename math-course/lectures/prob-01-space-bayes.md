...
> 概率论的第一页解决"概率是什么"：Kolmogorov 公理把它定义为满足三条规则的集合函数——从此概率论成为严格数学，其余一切（条件概率、独立性、Bayes）都是公理的推论。本页也是全站与 AI 课程交汇最密的一页：贝叶斯公式连接生成模型、分类与不确定性更新。
...
3. 只在本页虚构模型中，为什么阴性组的患病比例远低于阳性组？用 \(NPV=P(\bar D\mid-)\) 和四格计数各写一句，不要把 \(NPV\) 误读成 \(P(D\mid-)\)。

<details markdown="1">
<summary>迁移题核对：计数、赔率与条件独立</summary>

1. $FP=10000(0.99)(0.001)=9.9$ 是期望人数，$TP=99$，故 $PPV=99/108.9=10/11\approx90.9\%$。假阳性率从 $0.05$ 降为 $0.001$，缩小到原来的 $1/50$；健康组很大，减少这一项会显著改变分母。
2. 两个真实状态下都条件独立时，后验赔率为 $[\pi/(1-\pi)][s/(1-c)]^2$。默认值为 $(1/99)19.8^2=3.96$，后验概率 $3.96/4.96=99/124\approx79.8\%$。若第二次完全复制第一次的系统误差，则两次同阳性等于一次阳性事件，后验仍为 $1/6$，不能平方。
3. 阴性组共 $9406$ 个期望观测，其中患病 $1$、健康 $9405$，故 $P(D\mid-)=1/9406$，$NPV=9405/9406$。这些是给定模型下的组内比例，不是检测本身不随人群变化的“准确率”。

先修：[概率测度](real-01-measure.html)；后续：[随机变量与分布](prob-02-random-variables.html)、[条件期望](prob-04-moments.html)。
...
**两个古典模型**：古典概型 $P(A) = \frac{|A|}{|\Omega|}$（等可能有限，组合计数主战场）；几何概型 $P(A) = \frac{\text{测度}(A)}{\text{测度}(\Omega)}$（要求均匀分布且总测度有限、非零；例：会面问题）。Buffon 投针在针长 $0<l\le d$、平行线间距 $d$、针的方向及相对位置独立均匀时，命中概率为 $P=2l/(\pi d)$；长针不能照用此式。
...
这里阳性组约九成是假阳性，因为 $(1-\pi)(1-c)>\pi s$。**低基率与非零假阳性率要一起比较**：若特异度为 $1$ 且阳性事件有正概率，就没有假阳性，不能说“再准也一样”。
