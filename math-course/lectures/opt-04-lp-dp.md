...
容量 $b$ 改变后，旧活动集和旧对偶乘子都可能失效。先枚举新可行域的轴截点与约束交点，选出目标最大的连续候选 $x^*$；再求一个对偶可行 $u^*$，检查 $c^\top x^*=b^\top u^*$ 与互补松弛。只有这两本账同时通过，才是连续 LP 的最优证书。若变量必须为整数，连续最优值在最大化问题中只是上界；需要枚举、分支定界或本页的有界背包 DP 得到整数最优，不能把 $x^*$ 四舍五入后直接交卷。容量滑块改变时，实验会隐藏旧结果并要求重新预测，正是在模拟这张证书的失效。
