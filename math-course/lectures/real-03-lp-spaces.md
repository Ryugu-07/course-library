...
<details markdown="1"><summary>展开核对：因子来自测度和基向量</summary>
...
先修：[Lebesgue 积分](real-02-lebesgue-integral.html)、[Fourier 系数](analysis-04-series.html)；后续：[Hilbert 投影](func-02-hilbert.html)。
...
$L^2$ 的通常范数由以下**内积**诱导；在含两个互不相交、正且有限测度集合的非退化情形，其他 $p$ 的通常范数不满足平行四边形法则。一维空间等退化例外要单独看：
...
- **Parseval 等式** $\|f\|_2^2 = \sum_{n\in\mathbb Z} |c_n|^2$，其中 $c_n=\langle f,e^{inx}/\sqrt{2\pi}\rangle$——无穷维勾股定理（数分 IV 的预告兑现）；
