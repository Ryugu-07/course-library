...
<h3 id="arma-learning-title">1. 先预测：一条轨迹能替总体发证吗？</h3>

实验让 AR(1) 与 MA(1) 共用同一份固定随机回放。先判断：

1. $X_t=X_{t-1}+\varepsilon_t$ 的一条有限轨迹即便看起来围绕零，是否具有有限的平稳方差？
2. MA(1) 的**总体** ACF 在 lag $>1$ 精确为零，有限样本 ACF 是否也会逐项精确截尾？
3. 样本均值和方差在某个窗口内稳定，是否足以证明整个随机过程平稳？
