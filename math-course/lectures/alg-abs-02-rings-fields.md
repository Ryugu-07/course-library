...
下面的实验可改变 \(n\)，并切换三个二次商环。先预测，不要把有限列表当成一般证明：

1. 在 \(\mathbb Z_n\) 中，余数 \(2\) 什么时候是单位？
2. 非零零因子与 \(\gcd(a,n)\) 的关系是哪一种？
3. 把多项式模掉以后，当前商环还是域吗？
...
若 \(d\mid n\)，则 \((d)\) 是 \(\mathbb Z_n\) 的理想，并且 \(\mathbb Z_n/(d)\cong\mathbb Z_d\)。这是一个由具体生成元得到的商环证书，不意味着每个环的理想都长成这种形式。

对 \(f(x)\in\mathbb F_p[x]\)，商环 \(\mathbb F_p[x]/(f)\) 在 \(f\) 不可约时才是域；二次不可约例子给出扩张 \(\mathbb F_{p^2}\)，阶为 \(p^2\)。可约多项式会留下非零零因子。实验把每个元素逐项乘一遍，是有限模型的审计，不是“枚举证明所有环都如此”。
...
**定理** $\mathbb{Z}_p$ 是域 $\iff p$ 素。（*证*：$p$ 素时 $\gcd(a, p) = 1$，Bézout 给逆元；合数时有零因子。）**有限域 $\mathbb{F}_p$** 由此诞生；一般有限域阶必为素数幂 $p^k$，且在同构意义下每阶恰一个（$\mathbb{F}_{p^k}$，用 $\mathbb{F}_p[x]$ 模不可约多项式构造——多项式环再立一功）。
...
**Galois 理论（一段话的山顶导览）**：给每个多项式配一个群（根的对称群=Galois 群）；**方程可用根式求解 $\iff$ 其 Galois 群"可解"**（可逐层拆成 Abel 商的群）。$n \leq 4$ 时 $S_n$ 可解——二三四次有通用求根公式；对每个 $n\geq5$，$S_n$ 都含有不可解的 $A_n$，所以一般 $n$ 次方程没有根式公式。特别地，$S_5$ 含不可解的 $A_5$ 已足以判定**一般五次方程无根式解**（Abel–Galois，高代 I 的欠条在此销账）。注意量词：“一般”不等于“每个”；某些五次或更高次方程仍可用根式解。深层寓意：**"解方程"的可能性被"对称性结构"判定**——把问题翻译成结构、由结构宣判命运，这正是抽象代数（乃至现代数学）的方法论本体。
...
进一步学习：[从向量空间到模](../../grad-math/site/bridge-01-modules.html)会检验整数上为何不能任意除以主元，再用[局部化](../../grad-math/site/bridge-02-localization.html)解释指定分母怎样改变一个环。
