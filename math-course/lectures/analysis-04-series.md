...
<details markdown="1"><summary>迁移题核对：端点值与一致收敛的不同作用</summary>
...
先修回链：[积分与微积分基本定理](analysis-03-integral.html)；后续：[L² 与正交展开](real-03-lp-spaces.html)。
...
2. **逐项积分**：若各 $u_n$ 在有限闭区间 $[a,b]$ 上 Riemann 可积，且原级数一致收敛，则 $S$ 可积且 $\int_a^b S = \sum \int_a^b u_n$；
3. **逐项求导**：若各 $u_n\in C^1[a,b]$，$\sum u_n'$ 在 $[a,b]$ 一致收敛，且 $\sum u_n(x_*)$ 在至少一个 $x_*\in[a,b]$ 收敛，则原级数在 $[a,b]$ 一致收敛，和函数在内部可导且 $S'=\sum u_n'$。这里给的是便于使用的充分条件。

为什么需要那个锚点 $x_*$？对 $C^1$ 项，有

$$
\sum_{n=N}^{M}u_n(x)=\sum_{n=N}^{M}u_n(x_*)+\int_{x_*}^{x}\sum_{n=N}^{M}u_n'(t)\,dt.
$$

锚点控制常数项；导数尾和的一致小量再乘区间长度，控制所有 $x$。若漏掉锚点，$u_n(x)\equiv1$ 的导数级数恒为零，原级数却发散。对一般可导项也有更广版本，见[Waterloo 讲义定理 3.16](https://www.math.uwaterloo.ca/~snew/PMATH331/Chap3SequencesOfFunctions.pdf)。
