...
**① 无偏性**：$E\hat\theta = \theta$（瞄得正）。$\bar X$ 对 $\mu$ 无偏；$S^2$ 对 $\sigma^2$ 无偏而 $\hat\sigma^2_{\mathrm{MLE}}$ 偏小（差因子 $\frac{n-1}{n}$，大样本消失——"渐近无偏"）。

**② 有效性**：同为无偏，方差小者优（瞄得正还要抖得少）。

**③ 相合性**：$\hat\theta_n \xrightarrow{P} \theta$（样本无限多时收敛到真值——底线要求，矩法与 MLE 在正则条件下都满足；依据是大数定律）。
