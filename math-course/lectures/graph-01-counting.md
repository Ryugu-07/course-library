# 图论与组合 I · 计数：从容斥到生成函数

> 组合数学回答"有多少种"。本页是计数的兵器谱，按火力递增排列：基本原理 → 容斥 → 递推 → **生成函数**（把计数问题变成幂级数代数——组合与分析的握手，数分 IV 的幂级数在此再就业）。

<div data-learning-page></div>

<section class="learning-layer" markdown="1" aria-labelledby="graph-counting-learning-title">

## 学习层：先声明“数的对象”，再开始数

<h3 id="graph-counting-learning-title">1. 具身情境：为园区路线和树形网络开两本账</h3>

园区有一张小图。调度员问“从入口走 \(k\) 步有多少条路线”，网络工程师问“这张标号网络有多少棵生成树”，数据分析师又问“递推序列的第 \(m\) 项是多少”。三个问题都在数，但对象不同：简单路径不重复顶点，游走允许重复；握手引理数的是边端点；Cayley 公式数的是完全图的标号生成树；生成函数数的是某个序列的系数。
实验台可改变图型、顶点数、步长和生成函数系数指标。先预测：

1. 长度 \(k\) 的简单路径能否重复顶点？
2. 所有顶点度数之和是 \(E\)、\(2E\) 还是 \(nE\)？
3. \(n^{n-2}\) 数的是完全图 \(K_n\) 的什么对象？

<h3>2. 正式理论桥：四种计数各有自己的账本</h3>

固定起点并把顶点序列视为有序时，长度 \(k\) 的简单路径是 \(v_0,\ldots,v_k\)，其中顶点互异；游走只要求相邻顶点有边，可以回到已经访问过的顶点。实验用回溯逐条列举简单路径，用动态规划累计游走数，因此重复计数规则是显式的。

对有限无向图，握手引理是

$$
\sum_{v\in V}\deg(v)=2\lvert E\rvert.
$$

它按边的两个端点双计数，不是在数路径。树是连通无圈图；\(n\) 个标号顶点的树有 \(n-1\) 条边，而 Cayley 公式 \(n^{n-2}\) 数的是 \(K_n\) 上的标号树，不是任意图上的路线数。一般图的生成树数是 \(\tau(G)\)，可由 Matrix–Tree 定理的行列式得到。

生成函数把一个已经声明好的序列打包。例如

$$
F(x)=\frac{1}{1-x-x^2}
$$

的系数满足 \(a_0=1\)、\(a_1=1\)、\(a_m=a_{m-1}+a_{m-2}\)。这个系数账本与图上的路径计数可以有联系，但不能因为两者都出现递推就把它们当成同一个对象。

<div class="learning-lab" data-learning-lab="graph-counting" markdown="1">

**JavaScript 失效时的静态读法：**在路径图 \(P_4\) 中，从顶点 \(0\) 走两步有一条简单路径 \(0\to1\to2\)，但有两条游走 \(0\to1\to0\) 与 \(0\to1\to2\)。其度数和为 \(1+2+2+1=6=2\cdot3\)，它本身是一棵树且生成树数为 \(1\)；\(4^{4-2}=16\) 则是 \(K_4\) 的标号生成树数，不是 \(P_4\) 的路线数。
| 对象 | 当前有限证书 | 不能偷换成 |
|---|---|---|
| 简单路径 / 游走 | 顶点序列回溯与动态规划分别计数 | 把允许回访的游走当成无重复路径 |
| 握手引理 | 度数和 \(=2E\) | 路径数量或树的数量 |
| 树 / 生成树 | \(P_4\) 的树证书与 \(\tau(P_4)=1\) | Cayley 公式对任意图直接套用 |
| Cayley 公式 | \(K_4\) 的标号树数 \(4^2=16\) | \(P_4\) 的路线数 |
| 生成函数 | \([x^4]1/(1-x-x^2)=5\) | 把系数 5 当成图上某条路径的证明 |

</div>

<h3>3. 定理级结论与失败边界</h3>

- **对象先于公式：**必须先说明起点是否固定、路线是否有向、顶点是否允许重复；换一个约定，答案就可能改变。
- **双计数边界：**握手引理是每条无向边贡献两个端点的恒等式；度和为偶只是必要信息，不能单独证明某个度序列可实现。
- **树计数边界：**Cayley 的 \(n^{n-2}\) 只针对完全图上的标号生成树。一般图要数其生成树子图，不能把完全图公式当作通用枚举器。
- **生成函数边界：**系数提取是对已定义序列的代数编码；有限项实验能核对系数，却不能把数值枚举冒充双计数或递推正确性的证明。

</section>


<figure class="plot" markdown="1">
![Pascal 三角形 + 组合恒等式的图形（可 matplotlib 画数字三角 + 高亮路径）。](assets/img/graph-01-pascal.svg)
<figcaption><span class="fig-id">图 graph-01.1</span>Pascal 三角形 + 组合恒等式的图形（可 matplotlib 画数字三角 + 高亮路径）。</figcaption>
</figure>

## 1. 基本武器

**加法/乘法原理**（分类相加、分步相乘）；**排列** $A_n^k = \frac{n!}{(n-k)!}$、**组合** $\binom{n}{k}$；可重复组合（隔板法）$\binom{n+k-1}{k}$——"$k$ 个球放 $n$ 个盒"四种变体（球盒是否可辨）是一切初等计数题的分类框架。

**二项式定理**与组合恒等式两板斧：**代数法**（展开对比系数）与**组合双计数**（同一集合两种数法）。样板：$\binom{n}{k} = \binom{n-1}{k} + \binom{n-1}{k-1}$（Pascal：按"含不含第 $n$ 个元素"分类）；$\sum_k \binom{n}{k}^2 = \binom{2n}{n}$（Vandermonde：从 $2n$ 人选 $n$，按前半选几个分类）。

**鸽笼原理**：$n+1$ 物入 $n$ 笼必有同笼——弱得不能再弱的前提，专出漂亮结论（例：任意 $n+1$ 个整数中必有两个差被 $n$ 整除——按余数入笼，概率 I 生日问题的确定性表亲）。

## 2. 容斥原理

$$
\Big|\bigcup_{i=1}^n A_i\Big| = \sum|A_i| - \sum_{i<j}|A_i\cap A_j| + \cdots + (-1)^{n-1}|A_1\cap\cdots\cap A_n|
$$

（概率 I 加法公式的计数版；证明：每个元素被数的净次数 $= 1 - (1-1)^m = 1$，二项式定理一行。）

**旗舰应用（错排数）**：$n$ 封信全装错信封的方案数——容斥扣掉"至少某封装对"：

$$
D_n = n!\sum_{k=0}^{n}\frac{(-1)^k}{k!} \approx \frac{n!}{e}
$$

**全装错的概率 $\to \frac1e \approx 36.8\%$**——与 $n$ 几乎无关（10 封信与一万封信答案相同），组合数学第一名场面；$e$ 的级数（数分 IV）在纯计数问题里现身。

## 3. 递推关系

把"规模 $n$ 的方案数"用小规模表达。**线性递推的特征根法**（与 ode-02 常系数方程完全同构——差分方程即离散 ODE）：Fibonacci $F_n = F_{n-1} + F_{n-2}$，特征方程 $x^2 = x + 1$，根 $\varphi = \frac{1+\sqrt5}{2}, \psi = \frac{1-\sqrt5}{2}$：

$$
F_n = \frac{\varphi^n - \psi^n}{\sqrt 5}
$$

（黄金比例从"爬楼梯计数"里长出来；$|\psi| < 1$ ⇒ $F_n$ 是 $\frac{\varphi^n}{\sqrt5}$ 的四舍五入——增长率由最大特征根统治，ode-03/数值幂法/Markov 链的那句话第四次出场。）

## 4. 生成函数（计数的核武器）

**定义**：序列 $\{a_n\}$ 的（普通）生成函数 $G(x) = \sum a_n x^n$——**把整个数列打包成一个函数**，计数运算变函数代数：

| 组合操作 | 生成函数操作 |
|---|---|
| 两类选择的组合（先 A 后 B） | $G_A(x)\cdot G_B(x)$（卷积——概率 IV 母函数同款机制） |
| "每种物品选若干"的背包 | 各物品因子连乘 $\prod\frac{1}{1-x^{w_i}}$ 类 |
| 解线性递推 | 递推式 × $x^n$ 求和 ⇒ 关于 $G$ 的代数方程 |

**样板一（找零钱）**：用 1、2、5 分凑 $n$ 分的方案数 = $\frac{1}{(1-x)(1-x^2)(1-x^5)}$ 的 $x^n$ 系数——"枚举"变"展开"。

**样板二（Catalan 数）**：合法括号序列数 $C_n$ 满足 $C_{n+1} = \sum_k C_k C_{n-k}$（按最外层配对位置分类——卷积形！）⇒ $G = 1 + xG^2$，解二次方程 + 二项级数展开：

$$
C_n = \frac{1}{n+1}\binom{2n}{n} \qquad (1, 1, 2, 5, 14, 42, \dots)
$$

Catalan 数是组合的"$\pi$"：括号序列、二叉树形态、凸多边形三角剖分、不穿越对角线的格路……几十种问题同一个答案（栈的出栈序列数——CS 对账）。

🔗 **对账**：概率母函数（概率 IV）= 生成函数在 $a_n$ 为概率时的特例（独立和→乘积即卷积规则）；特征函数是它的 Fourier 版；分治算法复杂度递推（主定理背后）同属递推家族。

## 5. 典型例题

**例 1（隔板法）** 方程 $x_1 + x_2 + x_3 = 10$ 的非负整数解数：$\binom{10 + 2}{2} = 66$（10 球 2 板）。

**例 2（容斥）** 1–1000 中不被 2、3、5 任一整除的数：$1000 - (500 + 333 + 200) + (166 + 100 + 66) - 33 = 266$（Euler 函数思想的具体版——抽代 II 的 $\varphi$ 认亲）。

**例 3（生成函数解递推）** $a_n = 2a_{n-1} + 1,\ a_0 = 0$（汉诺塔）：两边乘 $x^n$ 求和得 $G = \frac{x}{(1-x)(1-2x)}$，部分分式展开（数分 III 的技术）得 $a_n = 2^n - 1$。$\blacksquare$

---

*下一页：从"数数"到"结构"——图的语言：度、连通、树、Euler 与 Hamilton、平面图。*
